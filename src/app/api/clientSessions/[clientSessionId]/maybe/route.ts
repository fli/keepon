...
type HandlerContext = RouteContext<'/api/clientSessions/[clientSessionId]/maybe'>
...
export async function POST(request: NextRequest, context: HandlerContext) {
  ...
  if (rawBodyText.trim().length > 0) {
    ...
    try {
      ...
      console.error('Failed to parse client session maybe request JSON', clientSessionId, error)
  ...
  const authorization = await authenticateTrainerRequest(request, {
    extensionFailureLogMessage: 'Failed to extend access token expiry while updating client session to maybe',
  })
  ...
  try {
    ...
    console.error('Failed to update client session to maybe', {
