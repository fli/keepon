...
export async function PUT(request: NextRequest, context: HandlerContext) {
  ...
  if (!paramsResult.success) {
    ...
    return NextResponse.json(
      buildErrorResponse({
        ...
        detail: detail || 'Client session note path parameters did not match the expected schema.',
  ...
  try {
    ...
    console.error('Failed to parse client session note update request body', clientSessionId, error)
  ...
  const authorization = await authenticateTrainerRequest(request, {
    extensionFailureLogMessage: 'Failed to extend access token expiry while updating client session note',
  })
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        buildErrorResponse({
          ...
          detail: 'Client session note response did not match the expected schema.',
