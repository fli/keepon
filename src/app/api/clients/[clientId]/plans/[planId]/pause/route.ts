...
export async function PUT(request: NextRequest, context: HandlerContext) {
  ...
  const authorization = await authenticateTrainerRequest(request, {
    extensionFailureLogMessage: 'Failed to extend access token expiry while pausing subscription',
  })
  ...
  try {
    ...
    console.error('Failed to pause subscription', authorization.trainerId, clientId, planId, error)
