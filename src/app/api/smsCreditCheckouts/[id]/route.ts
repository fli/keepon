...
const paramsSchema = z.object({
  id: z.string().trim().min(1, 'SMS credit checkout identifier is required.'),
})
...
const rawCheckoutDetailsSchema = z.object({
  email: z
    ...
    .pipe(z.string().min(1, 'Trainer email is missing for this SMS credit checkout.')),
  smsCreditBalance: z
    .union([z.number(), z.string()])
    .transform((value) => (typeof value === 'number' ? value : Number.parseFloat(value)))
    .refine((value) => Number.isFinite(value), 'SMS credit balance must be a finite number.'),
  country: z
    .union([z.string(), z.null()])
    .transform((value) => value ?? '')
    .pipe(
