...
const sanitizeQueryParams = (request: Request): QueryParams | NextResponse => {
...
export async function GET(request: Request) {
  const paramsOrResponse = sanitizeQueryParams(request)
  ...
  try {
    const rows = await fetchProducts(authorization.trainerId, paramsOrResponse)

    const products = productListSchema.parse(rows.map(mapRowToProduct))
