\restrict VWRqHuhPaQbfK9Sn5PuxExtWmKPAk4uDaEeSEu19a0yaUglJttqiXZDQKL8fHAf
...
CREATE TABLE public.trainer (
    ...
    online_bookings_duration_until_booking_window_opens interval DEFAULT '1 day'::interval CONSTRAINT trainer_online_bookings_duration_until_booking_window__not_null NOT NULL,
    online_bookings_duration_until_booking_window_closes interval DEFAULT '1 mon'::interval CONSTRAINT trainer_online_bookings_duration_until_booking_window_not_null1 NOT NULL,
    ...
    default_service_provider_appointment_reminder_1_type text DEFAULT 'emailAndNotification'::text CONSTRAINT trainer_default_service_provider_appointment_reminder__not_null NOT NULL,
    default_service_provider_appointment_reminder_2_type text DEFAULT 'emailAndNotification'::text CONSTRAINT trainer_default_service_provider_appointment_reminder_not_null1 NOT NULL,
...
CREATE TABLE public.payment_scheduled_stripe_attempt (
    ...
    id uuid DEFAULT public.uuid_generate_v1mc() NOT NULL,
    trainer_id uuid NOT NULL,
    payment_scheduled_stripe_id uuid CONSTRAINT payment_scheduled_stripe_at_payment_scheduled_stripe_i_not_null NOT NULL
);
...
CREATE VIEW public.vw_first_card_payments AS
 SELECT trainer_id,
    min(card_payment_date) AS first_card_payment_processed
...
CREATE VIEW public.vw_legacy_session AS
 SELECT id,
    timezone,
    to_char(timezone((timezone)::text, start), 'YYYY-MM-DD HH24:MI:SS'::text) AS date,
    (date_part('epoch'::text, duration) / (3600)::double precision) AS length,
    session_series_id AS "sessionSeriesId",
    ...
    booked_online AS "bookedOnline",
    maximum_attendance AS "maximumAttendance",
    ...
    google_place_id AS "googlePlaceId",
        CASE
            WHEN (geo IS NOT NULL) THEN json_build_object('lat', geo[0], 'lng', geo[1])
            ELSE NULL::json
        END AS geo,
    booking_payment_type AS "bookingPaymentType",
        CASE
            WHEN (service_provider_reminder_1 IS NULL) THEN NULL::json
            ELSE json_build_object('type', service_provider_reminder_1_type, 'timeBeforeStart', (service_provider_reminder_1)::text)
        END AS "serviceProviderReminder1",
        CASE
            WHEN (service_provider_reminder_2 IS NULL) THEN NULL::json
            ELSE json_build_object('type', service_provider_reminder_2_type, 'timeBeforeStart', (service_provider_reminder_2)::text)
        END AS "serviceProviderReminder2",
        CASE
            WHEN (client_reminder_1 IS NULL) THEN NULL::json
            ELSE json_build_object('type', client_reminder_1_type, 'timeBeforeStart', (client_reminder_1)::text)
        END AS "clientReminder1",
        CASE
            WHEN (client_reminder_2 IS NULL) THEN NULL::json
            ELSE json_build_object('type', client_reminder_2_type, 'timeBeforeStart', (client_reminder_2)::text)
        END AS "clientReminder2",
    buffer_minutes_before AS "bufferMinutesBefore",
    buffer_minutes_after AS "bufferMinutesAfter",
    bookable_online AS "bookableOnline",
    description,
    can_clients_cancel AS "canClientsCancel"
   FROM public.session;
...
CREATE TRIGGER queue_mandrill_event AFTER INSERT ON mandrill.event FOR EACH ROW WHEN ((new.processed_at IS NULL)) EXECUTE FUNCTION public.queue_mandrill_event();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON mandrill.event FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER add_sms_credits_on_renewal AFTER INSERT ON public.app_store_transaction FOR EACH ROW EXECUTE FUNCTION public.add_sms_credits_on_renewal();
...
CREATE TRIGGER client_trim BEFORE INSERT OR UPDATE ON public.client FOR EACH ROW EXECUTE FUNCTION public.client_trim();
...
CREATE TRIGGER send_mail AFTER INSERT ON public.mail FOR EACH ROW WHEN (((new.queued_at IS NULL) AND (new.mandrill_message_id IS NULL) AND (new.rejected_at IS NULL) AND (new.sent_at IS NULL))) EXECUTE FUNCTION public.send_mail();
...
CREATE TRIGGER send_sms AFTER INSERT ON public.sms FOR EACH ROW WHEN (((new.queued_at IS NULL) AND (new.twilio_message_sid IS NULL) AND (new.queue_failed_at IS NULL))) EXECUTE FUNCTION public.send_sms();
...
CREATE TRIGGER set_email_appointment_reminder_client_null_on_delete BEFORE DELETE ON public.client FOR EACH ROW EXECUTE FUNCTION public.set_email_appointment_reminder_client_null_on_delete();
...
CREATE TRIGGER set_email_appointment_reminder_session_null_on_delete BEFORE DELETE ON public.session FOR EACH ROW EXECUTE FUNCTION public.set_email_appointment_reminder_session_null_on_delete();
...
CREATE TRIGGER set_mail_client_null_on_delete BEFORE DELETE ON public.client FOR EACH ROW EXECUTE FUNCTION public.set_mail_client_null_on_delete();
...
CREATE TRIGGER set_sms_client_null_on_delete BEFORE DELETE ON public.client FOR EACH ROW EXECUTE FUNCTION public.set_sms_client_null_on_delete();
...
CREATE TRIGGER task_queue_insert AFTER INSERT ON public.task_queue FOR EACH ROW EXECUTE FUNCTION public.notify_task_queue_insert();
...
CREATE TRIGGER trainer_trim BEFORE INSERT OR UPDATE ON public.trainer FOR EACH ROW EXECUTE FUNCTION public.trainer_trim();
...
CREATE TRIGGER unattach_deleted_sale_from_client_session BEFORE DELETE ON public.sale FOR EACH ROW EXECUTE FUNCTION public.unattach_deleted_sale_from_client_session();
...
CREATE TRIGGER unattach_deleted_service_from_sessions BEFORE DELETE ON public.service FOR EACH ROW EXECUTE FUNCTION public.unattach_deleted_service_from_sessions();
...
CREATE TRIGGER update_appointment_reminder_before_insert BEFORE INSERT ON public.session FOR EACH ROW EXECUTE FUNCTION public.update_appointment_reminder_check();
...
CREATE TRIGGER update_appointment_reminder_before_update BEFORE UPDATE OF service_provider_reminder_1, service_provider_reminder_2, client_reminder_1, client_reminder_2, start ON public.session FOR EACH ROW EXECUTE FUNCTION public.update_appointment_reminder_check();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.access_token FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.analytics_data FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.app_notification FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.app_store_pending_renewal_info FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.app_store_server_notification FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.app_store_transaction FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.availability FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.busy_time FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.client FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.client_login_request FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at AFTER UPDATE ON public.client_note FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.client_payment_reminder FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.client_session FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.credit_pack FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.email_appointment_reminder FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.finance_item FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.installation FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.mail FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.mail_bounce FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.mail_click FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.mail_open FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.mission FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_credit_pack FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_manual FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_method FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_plan FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_plan_acceptance FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_plan_charge FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_plan_pause FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_plan_payment FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_scheduled_stripe FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_scheduled_stripe_attempt FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_stripe FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.payment_subscription FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.product FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.reward FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.sale FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.sale_credit_pack FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.sale_item FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.sale_product FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.sale_service FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.service FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.session FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.session_series FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.sms FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.sms_credit FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.sms_credit_checkout_session FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.stripe_balance FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.stripe_charge FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.stripe_payment_intent FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.stripe_resource FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.subscription FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.subscription_pause FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.subscription_period FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.subscription_update FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.survey FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.trainer FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.trial FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON public.user_ FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER process_stripe_event AFTER INSERT ON stripe.event FOR EACH ROW WHEN ((new.processed_at IS NULL)) EXECUTE FUNCTION public.process_stripe_event();
...
CREATE TRIGGER save_account_external_accounts AFTER INSERT OR UPDATE ON stripe.account FOR EACH ROW EXECUTE FUNCTION public.save_account_external_accounts();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON stripe.account FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON stripe.bank_account FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON stripe.checkout_session FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON stripe.customer FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON stripe.event FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON stripe.payment_method FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON stripe.payout FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON stripe.subscription FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
CREATE TRIGGER update_updated_at BEFORE UPDATE ON twilio.message FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();
...
\unrestrict VWRqHuhPaQbfK9Sn5PuxExtWmKPAk4uDaEeSEu19a0yaUglJttqiXZDQKL8fHAf
